    ...
    public static class InnDialogueValidation
    {
        ...
        public static void Run()
        {
            ...
            try
            {
                ...
                game.opening.state.sawTroll=true;game.firstLab.Show();
                Choose(game,"That's a troll. You brought him into the inn?");
                Check(state.rescueKnown&&!game.opening.state.questAccepted,"surprise learns rescue without accepting work");
                Choose(game,"You pulled a troll out of the river?");
                Check(game.dialogue.text=="He was drowning.","surprised branch preserves locked rescue line");
                state.labCheckpoint="questions";
                game.opening.state.sawTroll=true;game.firstLab.Show();Check(game.dialogue.choices.Count==2&&!game.opening.state.questAccepted,"two focused patient branches");Choose(game,"What's wrong with him?");Check(state.symptomsKnown&&game.dialogue.text.StartsWith("I don't know."),"symptoms remembered");game.dialogue.continueAction();game.dialogue.continueAction();game.dialogue.continueAction();game.dialogue.continueAction();Check(state.marlowJob==MarlowJob.Offered&&state.ingredientsKnown&&!game.opening.state.questAccepted,"ingredients precede commitment gate");Check(!game.dialogue.choices.Exists(c=>c.label=="What are you paying?"),"upstairs payment question not repeated");
                Choose(game,"No.");Check(state.marlowJob==MarlowJob.Refused&&!game.opening.state.questAccepted,"job refusal distinct");game.CloseDialogue();game.firstLab.Show();Check(state.marlowJob==MarlowJob.Refused&&game.dialogue.choices.Exists(c=>c.label=="I'll get them."),"job refusal remembered while reconsideration stays available");Choose(game,"I'll get them.");Check(state.marlowJob==MarlowJob.Accepted&&game.opening.state.questAccepted&&game.dialogue.text=="Thank you.","only commitment accepts quest");game.dialogue.continueAction();Check(game.dialogue.choices.Count==0&&game.dialogue.continueAction!=null,"briefing returns through ordinary Continue");
